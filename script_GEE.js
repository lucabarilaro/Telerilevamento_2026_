...
var collection = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
                   .filterBounds(aoi)
                   .filterDate('2016-05-01', '2016-06-30')              // Filter by date                                   // Filter by AOI
...
Map.centerObject(aoi, 12); // Zoom to the AOI

// Display the first image of the collection (GEE does this by default)
Map.addLayer(collection, {
  bands: ['B8', 'B3', 'B2'],  // True color: Red, Green, Blue
  min: 0,
  max: 0.3
...
Export.image.toDrive({
  image: composite.select(['B2', 'B3', 'B4', 'B8']),  // Select RGB bands
  description: 'Giglio_Export_Task',
  folder: 'GEE_Giglio',                        // Folder in Google Drive
  fileNamePrefix: 'Giglio_pre_2016',
  region: aoi,
